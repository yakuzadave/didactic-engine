...
class TestChunkingPerformance:
    ...
    def test_5_minute_song_chunking_performance(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if "bar_features_parquet" not in results:
                pytest.skip("No bar features created - beats may not have been detected")
            ...
            print(f"  ✓ Performance test passed: {pipeline_time:.2f}s for {duration}s audio")
            ...
            for idx in sample_indices:
                if idx < len(df):
                    ...
                    if chunk_path.exists():
                        ...
                        assert len(chunk_audio) > 0, f"Empty chunk at bar {row['bar_index']}"
    ...
    def test_chunk_memory_efficiency(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            print(f"  ✓ Memory efficiency test passed: {len(df)} bars processed")
...
class TestChunkQuality:
    ...
    def test_chunk_boundaries_aligned_to_beats(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            beat_envelope = np.where(np.sin(2 * np.pi * beat_freq * t) > 0.9, 1.0, 0.3)
